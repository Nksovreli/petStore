...
class Pet(db.Model): #ბაზისთვის შექმნილი ცხოველის კლასი, რა ექნება  ბაზაში:აიდი , სახელი,სქესი,აღწერა.
    ...
    @staticmethod
    def get_all(): #ამოაქვს ბაზიდან ყველა ჩაწერილი ცხოველი. AVAILABLE PETS-ში რაც ჩანს მერე
        return Pet.query.all()
    
    def get_by_id(pet_id):#იღებს ბაზიდან ცხოველს აიდის მიხედვით რომელიც მერე გამოიყენება  კონკრეტული ცხ. დეტალების გამოსატანად
...
@app.route('/')#როცა შეხვალ საიტზე პირველივე გვერდი რაც გამოჩნდება ესაა ის 
def home():
    return render_template('home.html')

@app.route('/store', methods=['GET', 'POST'])#პირველ გვერდზე სტორეს რო დააწავები და მერე რაც გამოჩნდება ეგაა ეს
def pet_store():
...
@app.route('/donate', methods=['GET', 'POST'])#დონაციას ეს
def donate():
    if request.method == 'POST':
...
@app.route('/register', methods=['GET', 'POST'])#რეგისტრაციააა
def registration():
    if request.method == 'POST':
...
def get_pets():
    # Send a GET request to the API endpoint for available pets in another country
...
if __name__ == '__main__':  #ეს სტარტავს აპლიკაციას
    initialize_db(app)
    with app.app_context():
        db.create_all()
    app.run(debug=True)
